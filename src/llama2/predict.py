...
dataset = llm_science_exam.llama2.dataset.add_prompt_field(
    dataset, prompt_id=config["prompt_id"], new_field_name="text", with_answer=dataset_type == "train"
)
...
for dataset_type, example in tqdm.tqdm(
    ...
    perplexities = llm_science_exam.llama2.predict.calc_perplexities(model, tokenizer, samples)

    pred = np.take(cols, np.argsort(perplexities))
